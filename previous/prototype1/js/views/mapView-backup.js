...
//----------------------------------//
// NEW. Export Structure for main.js
//----------------------------------//
...
export function initMapView(nodes, links, generators, buses, lines) {
  allNodes = nodes;
  allLinks = links;
  ...
  // Snap gen/load positions, etc.
  snapGeneratorPositions();
  updateVisualization("manual");
}

export function updateMapView(systemState) {
  // Update visuals based on currentOutput
  d3.selectAll('.node-generator-manual circle')
    .attr('fill', d => {
      const genData = opGenerators.find(g => "Gen" + g.busNumber === d.id);
      return (genData?.currentOutput || 0) > 0 ? "#87e291" : "none";
    });
}
